...
command.action(async (_options: Options) => {
  ...
  try {
    ...
    if (gypFile.targets?.length === 1) {
      if (typeof gypFile.targets[0]?.target_name === "string") {
        guessedPackageName =
          gypFile.targets[0]?.target_name || guessedPackageName;
      }
    }
  } catch {}
  ...
  const buildTargets = userTargets
    ...
    .map((target) => ({
      ...target,
      targetId: [target.os, target.cpu, target?.variant]
        .filter((v) => v)
        .join("-"),
    }));
  ...
  buildTargets.forEach(async (target) => {
    ...
    await writeJson<PackageJson>(`${targetDir}/package.json`, {
      name: `${packageName}-${target.targetId}`,
      version: pkg.version,
      os: target.os,
      cpu: target.cpu,
      main: `${packageName}.${target.targetId}.node`,
      files: [`${packageName}.${target.targetId}.node`],
      engines: pkg.engines,
    });
  });

  // Install our "build" script in package.json
  const cmakeSrc = await convertGypToCmakeJs(
    `${projectRoot}/bindings.gyp`,
    packageName,
  );
  if (cmakeSrc) {
  ...
  buildTargets.forEach((target) => {
    ...
    const sysroot = execSync(`xcrun -sdk ${target.apple?.sdk} --show-sdk-path`)
      .toString()
      .trim();

    const appleCpus = target.cpu
      .map((cpu) => {
        switch (cpu) {
          case "x64":
            return "x86_64";
  ...
  const appleLibs: string[] = buildTargets
    ...
    .map(
      (t) =>
        `${projectRoot}/npm/${t.targetId}/build/Release/${packageName}.dylib`,
    );
  const xcframeworkCmd = [
    ...
    "-output",
    `xcframeworks/${packageName}.xcframework`,
  ].join(" ");

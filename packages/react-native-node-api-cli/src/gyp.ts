...
export async function convertGypToCmakeJs(
  gypFilePath: string,
  projectName: string,
) {
  const napiVersion: number = 4; // TODO: Read this from package.json?
  ...
  try {
    ...
    targets.forEach((target) => {
      ...
      cmakeSrc.push(
        ...
        ...["${CMAKE_JS_SRC}", ...sources].map(indent()),
        `)`,
      );

      cmakeSrc.push(
        `target_compile_definitions(${targetName} PRIVATE`,
        ...[`NAPI_VERSION=${napiVersion}`, ...defines].map(indent()),
        ")",
      );
      ...
      cmakeSrc.push(
        ...
        ...["${CMAKE_JS_SRC}", ...includeDirs].map(indent()),
        ")",
      );

      cmakeSrc.push(`target_link_libraries(${targetName} \${CMAKE_JS_LIB})`);
    });

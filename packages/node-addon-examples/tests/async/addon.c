...
static void Complete(napi_env env, napi_status status, void* data) {
  ...
  NODE_API_CALL_RETURN_VOID(
      env, napi_get_reference_value(env, c->_callback, &callback));
  napi_value global;
  NODE_API_CALL_RETURN_VOID(env, napi_get_global(env, &global));
  napi_value result;
  NODE_API_CALL_RETURN_VOID(
      env, napi_call_function(env, global, callback, 2, argv, &result));
...
static napi_value Test(napi_env env, napi_callback_info info) {
  ...
  void* data;
  NODE_API_CALL(env, napi_get_cb_info(env, info, &argc, argv, &_this, &data));
  ...
  NODE_API_ASSERT(
      env, t == napi_object, "Wrong second argument, object expected.");
  NODE_API_CALL(env, napi_typeof(env, argv[2], &t));
  NODE_API_ASSERT(
      env, t == napi_function, "Wrong third argument, function expected.");

  the_carrier._output = 0;

  NODE_API_CALL(env, napi_get_value_int32(env, argv[0], &the_carrier._input));
  NODE_API_CALL(
      env, napi_create_reference(env, argv[2], 1, &the_carrier._callback));
...
static void CancelComplete(napi_env env, napi_status status, void* data) {
  ...
  if (status == napi_cancelled) {
    ...
    napi_value result;
    NODE_API_CALL_RETURN_VOID(
        env, napi_call_function(env, global, callback, 0, NULL, &result));
  }
...
static napi_value TestCancel(napi_env env, napi_callback_info info) {
  ...
  NODE_API_CALL(
      env, napi_create_reference(env, argv[0], 1, &async_carrier[0]._callback));
...
static void RepeatedWorkComplete(napi_env env, napi_status status, void* data) {
  ...
  NODE_API_CALL_RETURN_VOID(
      env, napi_get_reference_value(env, repeated_work_info.ref, &cb));
  NODE_API_CALL_RETURN_VOID(
      env, napi_delete_async_work(env, repeated_work_info.work));
  NODE_API_CALL_RETURN_VOID(
      env, napi_delete_reference(env, repeated_work_info.ref));
  repeated_work_info.work = NULL;
  repeated_work_info.ref = NULL;
  NODE_API_CALL_RETURN_VOID(
      env, napi_create_uint32(env, (uint32_t)status, &js_status));
  NODE_API_CALL_RETURN_VOID(
      env, napi_call_function(env, cb, cb, 1, &js_status, NULL));
}
...
static napi_value DoRepeatedWork(napi_env env, napi_callback_info info) {
  ...
  NODE_API_CALL(
      env, napi_create_reference(env, cb, 1, &repeated_work_info.ref));
  NODE_API_CALL(env,
  ...
  NODE_API_CALL(env,
      napi_create_async_work(env,
          NULL,
          name,
          RepeatedWorkerThread,
          RepeatedWorkComplete,
          &repeated_work_info,
